  ...
  return (
    <section className='CallToAction'>
      <div className='action-items'>
        <h1 data-testid='cta-title'>{props.title || "Looks like you aren't signed in yet!"}</h1>
        {/*should a componet pass in a title it will render, otherwise the default message should*/}
        <p className='prompt' data-testid='cta-prompt'> Sign in if you already have an account with us,
        ...
        {/*Presumably Links are best practice vs a button with a state change on a redirect, but I am not sure*/}
          <Link to='/signin' className='options' data-testid='sign-in-link'> Sign In </Link>
          <Link to='/signup' className='options' data-testid='sign-up-link' > Sign Up </Link>
        </div>
      </div>
      <Footer
        path1='/'
